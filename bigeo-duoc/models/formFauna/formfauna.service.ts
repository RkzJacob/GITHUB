...
@Injectable()
export class FormFaunaService {
    constructor(
        @InjectModel(formFauna)
        private readonly formFaunaModel: typeof formFauna,private readonly sequelize: Sequelize
      ) {}
      ...
      async obtCompCerroTunel(Fecha1: string, Fecha2: string): Promise<any> {
        try {
          ...
          const query = `
            SELECT TO_CHAR("dateTime",'DD/MM/YYYY') as Fecha, fauna ,COUNT(fauna) as cantidad,
            sector as sector
            FROM public.form
            JOIN public.properties on propid="propertiesPropid"
            JOIN public."formFauna" on fauid="formFaunaFauid" 
      ...
      async obtCompCerroCasa(Fecha1: string, Fecha2: string): Promise<any> {
        try {
          ...
          if (!CerroCasa) {
            throw new Error('No se encontró el cerro "sectoresCerroCasa" en la estructura.');
          }
      ...
      async CerroTunel2(): Promise<any> {
        try {
          ...
          const query = `
            SELECT fauna as fauna, COUNT(fauna) as cantidad,
            sector as sector
            FROM public.form
            JOIN public.properties on propid="propertiesPropid"
            JOIN public."formFauna" on fauid="formFaunaFauid"
            WHERE sector IN (${placeholders}) 
            GROUP BY fauna,sector
            ORDER BY 1,2 ASC
          `;
