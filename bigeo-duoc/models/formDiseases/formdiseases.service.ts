...
@Injectable()
export class FormDiseasesService { 
    constructor(
        @InjectModel(formDiseases)
        private readonly formDiseasesModel: typeof formDiseases,private readonly sequelize: Sequelize
      ) {}
      ...
      async obtCompCerroTunel(Fecha1: string, Fecha2: string): Promise<any> {
        try {
          ...
          const query = `
            SELECT TO_CHAR("dateTime",'DD/MM/YYYY') as Fecha, diseases,COUNT(diseases) as cantidad,
            sector as sector
            FROM public.form
